...
public interface VotoUtilRepository extends JpaRepository<VotoUtil, Long> {
    ...
    @Query("SELECT v.review.id FROM VotoUtil v WHERE v.usuario.email = :email AND v.review.catedra.id = :catedraId")
    List<Long> reviewIdsVotadasPor(@Param("email") String email, @Param("catedraId") Long catedraId);
}
